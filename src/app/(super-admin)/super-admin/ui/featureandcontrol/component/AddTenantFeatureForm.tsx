...
export interface TenantFeatureFormData {
  portal: string;
  category: string;
...
export interface TenantParentModuleOption {
  id: string;
  name: string;
  children: { id: string; name: string }[];
}

export interface TenantPortalOption {
  id: string;
  name: string;
}

interface AddTenantFeatureFormProps {
  formData: TenantFeatureFormData;
  isLoading: boolean;
  portalOptions: TenantPortalOption[];
  parentModuleOptions: TenantParentModuleOption[];
  onClose: () => void;
  onReset: () => void;
...
const AddTenantFeatureForm = ({
  ...
  const selectedParentModule = parentModuleOptions.find(
    (option) => option.id === formData.parentModule,
  );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4 py-4">
      <div className="relative max-h-[95vh] w-full max-w-[700px] overflow-y-auto rounded-lg bg-white shadow-2xl">
        <button
          type="button"
          aria-label="Close add tenant feature form"
          onClick={onClose}
              ...
              {!isModule && (
                <>
                  ...
                  <div className="mb-3 flex items-center gap-3 rounded-md bg-[#E9EDFF] px-3 py-2 text-[11px] text-[#17244A]">
                    ...
                    Pick one or more child modules to add this feature under
                    them, or leave none selected to add it directly under the
                    parent module.
                  </div>
                  <div className="mb-3">
                    ...
                    <div className="flex flex-wrap gap-2 rounded-md border border-[#D6D8DE] p-2.5">
                      ...
                      ) : selectedParentModule.children.length === 0 ? (
                        <span className="px-1 py-1 text-[11px] text-gray-400">
                          No child modules under this parent - feature will be
                          added directly under it.
                        </span>
                      ) : (
                        selectedParentModule.children.map((child) => (
                          <button
                            key={child.id}
                            type="button"
...
export default AddTenantFeatureForm;
